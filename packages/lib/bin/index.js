...
program.arguments('[command]').action((cmd) => {
  ...
  if (cmd) {
    ...
    console.log(
      chalk.red(
        `  ⛔️ Unknown command ${chalk.bold.keyword('dodgerblue')(cmd)}.`,
      ),
    )
    console.log()
  }
...
program
  ...
  .option(
    ...
    'Optionnal: Provide path to config file - Ex: "./maz-ui.config.js"',
  )
  .description('Generate css variables')
  .action(async (options) => {
    const configPath = path.resolve(
      process.env.PWD,
      options.config ? options.config : './maz-ui.config.js',
    )
