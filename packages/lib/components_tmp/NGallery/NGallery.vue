<template>
  <section class="n-gallery">
    <NGalleryResponsive v-if="!$mq[responsiveBreakpoint]" :medias="mobileMedias" />
      ...
      <div class="n-gallery-desktop-column">
        <ol ref="nGalleryColumnElement" class="n-gallery-desktop-column-scroll">
          <template v-for="media in mediasWithId">
            ...
            >
              <NLazyImg v-if="media.srcs.small" :image="media.srcs.small" load-once />
            </li>
          </template>
        </ol>
...
<script lang="ts">
  ...
  import type { DataImage } from './../NLazyImg/NLazyImg.vue'
  ...
  const getMainMediaProps = (data: GalleryDataMedia) => {
    return isVideo(data.type) ? { url: data.url } : { image: data, 'no-use-loader': true }
  }
    ...
    setup(props) {
      ...
      const setMainImage = (image: GalleryMediaWithId) => {
        slideTransition.value =
          image.id > mainImage.value.id ? SlideTransition.UP : SlideTransition.DONW
