<template>
  ...
  >
    ...
    <CalendarBanner v-if="bannerMessage || $slots['banner-message']">
      <slot v-if="$slots['banner-message']" name="banner-message" />
      <template v-else>
        {{ bannerMessage }}
      </template>
    </CalendarBanner>
    ...
    >
      ...
      <CalendarEmptySlots
        v-else-if="!hasSlotsAvailable || !hasSlotsPageAvailable"
      >
        <span>
          ...
          <slot
            v-else-if="!hasSlotsPageAvailable"
            name="empty-slots-hours-text"
          >
    ...
    <CalendarFooter
      v-if="$slots['footer'] || $slots['footer-text']"
      :no-x-offset="noWeekSwitch"
    >
...
<script lang="ts">
    ...
    try {
      if (Array.isArray(value)) {
        const isValidPayload =
          value.length > 0 && value.every((slot) => !!slot.start && !!slot.end)
      ...
      console.error(new Error(`[Calendar] Invalid prop "value" - ${err}`))
      return false
    }
  }
  ...
  export default class Calendar extends Vue {
    ...
    get calendarMinWidth() {
      ...
      const weekSwitchColumnsWidth = this.noWeekSwitch
        ? 0
        : CALENDAR_WEEK_BUTTON_WIDTH * 2
    ...
    get daysWithSlotsFilteredPerPage() {
      return this.days.map((day) => ({
        ...
        slots: this.noSlotSwitch
          ...
          : day.slots?.slice(
              this.currentPage,
              this.currentPage + this.slotsCountPerPage,
            ),
      }))
    }

    get hasSlotsPageAvailable() {
      return this.daysWithSlotsFilteredPerPage.some(
        ({ slots }) => slots?.length,
      )
    }
    ...
    get maxSlotsReached() {
      return this.maxSlotsAllowed
        ? this.tmpSelectedSlots.length >= this.maxSlotsAllowed
        : false
    }
    ...
    onValueOrDaysChanged() {
      if (this.value && this.value?.length > 1) {
        const selectedDaysSlots = [
          ...this.tmpSelectedSlots,
          ...this.getSelectedSlotsInDays(),
        ]
    ...
    removeDuplicate(daySlots: DaySlot[]) {
      return daySlots.filter(
        (value, index, self) =>
          ...
          self.findIndex(
            (t) =>
              this.isSameDate(t.start, value.start) &&
              this.isSameDate(t.end, value.end),
          ),
      )
    }
    ...
    removeItem(selectedSlots: DaySlot[], slot: DaySlot): DaySlot[] {
      const index = selectedSlots.findIndex(
        ({ start, end }) =>
          this.isSameDate(slot.start, start) && this.isSameDate(slot.end, end),
      )
    ...
    selectSlot(slot: DaySlot) {
      ...
      if (alreadySelected) {
        ...
        this.selectedSlots =
          this.minSlotsReached && this.tmpSelectedSlots.length
            ? this.tmpSelectedSlots
            : undefined
        return
      }
