...
describe('given MazFullscreenLoader.vue component', () => {
  ...
  it('display the spinner with good properties', async () => {
    ...
    expect(teleportedContent?.innerHTML).toMatchInlineSnapshot(`"<svg width="2em" height="2em" version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 50 50" xml:space="preserve" class="m-spinner m-reset-css maz:animate-spin maz:fill-current maz:m-0! m-spinner--secondary" style="color: var(--maz-secondary);"><path d="M43.935,25.145c0-10.318-8.364-18.683-18.683-18.683c-10.318,0-18.683,8.365-18.683,18.683h4.068c0-8.071,6.543-14.615,14.615-14.615c8.072,0,14.615,6.543,14.615,14.615H43.935z"></path></svg><span>Contenu personnalisé</span>"`)
  ...
  it('adds class to document on mount and removes it on unmount', async () => {
    const wrapper = mount(MazFullscreenLoader, {
      props: { teleportSelector: '#teleport-target' },
    })
    ...
    expect(document.documentElement.classList.contains('--maz-fullscreen-loader-present')).toBe(true)

    wrapper.unmount()

    expect(document.documentElement.classList.contains('--maz-fullscreen-loader-present')).toBe(false)
  })
})
