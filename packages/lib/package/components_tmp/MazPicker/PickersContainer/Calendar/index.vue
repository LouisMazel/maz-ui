<template>
  <div
    :id="`Calendar${_uid}`"
    ref="Calendar"
    class="calendar maz-position-relative maz-mw-100 maz-flex maz-overflow-hidden"
    :class="{
  ...
  >
    ...
    <div
      v-if="hasDate"
      ref="MonthsContainer"
      class="calendar__months-container maz-flex-1 maz-overflow-hidden"
    >
      <MonthYearSwitcher
        :months="months"
        class="maz-px-2"
      ...
      <div class="maz-flex maz-overflow-x-auto">
        ...
        >
          <WeekDaysLabels :locale="locale" class="maz-p-2" />
          <MonthPicker
            ref="MonthPicker"
            v-model="dateMoment"
            :month="month"
    ...
    watch: {
      value: {
        handler(newValue, oldValue) {
          const newCurrentValue =
            this.isRangeMode && newValue
              ? newValue.end || newValue.start
              : newValue
          const oldCurrentValue =
            this.isRangeMode && oldValue
      ...
      months: {
        async handler() {
          ...
          this.contentHeight =
            MonthsContainer && MonthsContainer.clientHeight
              ? MonthsContainer.clientHeight
              : CONTENT_HEIGHT
        },
        immediate: true,
      },
    ...
    methods: {
      updateMonth() {
        ...
        this.months = this.getMonths({
          year: currentYear,
          month: hasRangeValuesOnDifferentsMonths
            ? currentMonth - 1
            : currentMonth,
        })
      },
