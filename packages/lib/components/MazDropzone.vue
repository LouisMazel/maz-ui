...
import { useDropZone } from './../modules/composables/useDropzone'
import { sleep } from './../modules/helpers/sleep'
import MazBtn, { type Props as MazBtnProps } from './MazBtn.vue'
import MazIcon from './MazIcon.vue'
import MazSpinner, { type MazSpinnerProps } from './MazSpinner.vue'
...
const emits = defineEmits<{
   ...
   * @property {{ files: File[] | null, event: DragEvent | null, code: ErrorCode }} values - The files, event and error code
   */
  'error': [values: { files: File[] | null, event: DragEvent | null, code: ErrorCode }]
  /**
   * Event emitted when an error occurs during file upload
   * @property {{ file: File, code: ErrorCode, error: unknown }} values - The file, error code and error details
   */
  'upload-error': [values: { file: File, code: ErrorCode, error: unknown }]
  /**
   * Event emitted when an error occurs during multiple files upload
   * @property {{ files: File[], code: ErrorCode, error: unknown }} values - The files, error code and error details
   */
  'upload-error-multiple': [values: { files: File[], code: ErrorCode, error: unknown }]
  /**
...
export type ErrorCode = 'FILE_SIZE_EXCEEDED' | 'MAX_FILES_EXCEEDED' | 'FILE_TYPE_NOT_ALLOWED' | 'FILE_DUPLICATED' | 'FILE_UPLOAD_ERROR' | 'NO_FILES_TO_UPLOAD' | 'FILE_UPLOAD_ERROR_MULTIPLE' | 'NO_URL'
...
const filesData = defineModel<FileData[]>({
  default: () => [],
})
...
async function getFileData(file: File): Promise<FileData> {
...
async function handleFiles(files: File[] | FileList | null) {
  ...
  for await (const file of files) {
    if (dataTypes && !dataTypes.includes(file.type)) {
      emits('error', {
        files: [file],
        event: null,
...
function getFileIcon(fileData: FileData) {
...
function handleFileRemove(fileData: FileData) {
  filesData.value = filesData.value.filter(f => f.name !== fileData.name && f.size !== fileData.size && f.type !== fileData.type)
...
function removeFile(file: File) {
  filesData.value = filesData.value.filter(f => f.name !== file.name)
}
  ...
  >
    ...
    <slot name="files-area" :files-data="filesData">
      <TransitionGroup name="file-scale" tag="div" class="m-dropzone__files-container" @before-enter="selectAreaCanBeDisplayed = false" @after-leave="selectAreaCanBeDisplayed = filesData.length === 0">
        ...
        <div v-for="file in filesData" :key="file.name" class="m-dropzone__file-item group" @click.prevent="">
          <!--
