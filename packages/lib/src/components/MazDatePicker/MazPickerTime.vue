...
const MazDatePickerTime = ref<HTMLDivElement>()
const spacerHeight = ref<number>()
...
function setSpacerHeight() {
  ...
  const item = MazDatePickerTime.value.querySelector(
    `.m-date-picker-time__column .m-btn`,
  ) as HTMLButtonElement
  const itemHeight = item?.offsetHeight
  const timePickerHeight = MazDatePickerTime.value.offsetHeight

  const topSpace = timePickerHeight / 2 - itemHeight / 2
...
async function scrollColumn(identifier: ColumnIdentifier, hasSmoothEffect = true) {
  ...
  const column = MazDatePickerTime.value.querySelector(`.m-date-picker-time__column__${identifier}`) as
    | HTMLDivElement
    | undefined

  const selectedButton = MazDatePickerTime.value.querySelector(
    `.m-date-picker-time__column__${identifier} .--is-selected`,
  ) as HTMLButtonElement | undefined

  if (spacerHeight.value && column && selectedButton && MazDatePickerTime.value) {
    await nextTick()
    scrollToTarget(column, selectedButton, spacerHeight.value, hasSmoothEffect)
  }
}
