  ...
  export default {
    ...
    computed: {
      ...
      firstOne () {
        ...
        return (currentPage - halfPageRange) > 1
          ? allPages.slice(0, 1)
          : []
      },
      lastOne () {
        const { allPages, halfPageRange, currentPage, pageCount } = this
        return currentPage < (pageCount - halfPageRange)
          ? allPages.slice(-1)
          : []
      },
      rangeStartAt () {
      ...
      firstDivider () {
        ...
        return (currentPage - halfPageRange) > 2
          ? divider
          : []
      },
      lastDivider () {
        const { currentPage, halfPageRange, pageCount } = this
        return currentPage < (pageCount - halfPageRange - 1)
          ? divider
          : []
      },
      pages () {
